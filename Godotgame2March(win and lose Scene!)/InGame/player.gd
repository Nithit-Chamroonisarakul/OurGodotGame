...
func shoot():
	...
	await get_tree().create_timer(0.1).timeout  # Wait for cooldowncan_shoot = true  # Allow shooting again	
	can_shoot = true
...
func die(): #new
	if is_alive == false:
		...
		get_tree().change_scene_to_file("res://InGame/game_over_scene.tscn")
